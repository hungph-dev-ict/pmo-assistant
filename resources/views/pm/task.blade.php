...
@section('content')
    <div data-vue-app="task-container" data-project-id="{{ $project_id }}" data-current-user-id="{{ auth()->user()->id }}"
        data-current-user-account="{{ auth()->user()->account }}" data-user-role="{{ auth()->user()->getRoleNames() }}"
        data-project-audit='@json($project_audit)'></div>
@endsection
