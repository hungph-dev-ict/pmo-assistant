    ...
    <div class="card card-success collapsed-card">
        ...
        <div class="card-body">
            <div class="form-group">
                <label for="fileInput">Select CSV File <span style="color: red">*</span></label>
        ...
        <div class="card-footer">
            ...
            <button v-else type="button" class="btn btn-primary" @click="submitFile"
...
const parsedListAssignee = computed(() => {
    const list = typeof props.listAssignee === "string" ? JSON.parse(props.listAssignee) : props.listAssignee;
...
const statusMap = { "Open": 0, "In Progress": 1, "Resolved": 2, "Feedback": 3, "Done": 4 };
...
const processCsv = (csvText) => {
    ...
    if (lines.length < 2) {
        validationErrors.value.push("⚠️ CSV file must have at least one data row.");
        return;
    }
    ...
    if (!requiredHeaders.every(h => headers.includes(h))) {
        validationErrors.value.push(`⚠️ CSV file must contain: ${requiredHeaders.join(", ")}.`);
        return;
    }

    lines.slice(1).forEach((row, index) => {
        const lineNumber = index + 2;
        const rowData = Object.fromEntries(headers.map((header, i) => [header, row[i] || null]));
        ...
        if (!epic || !assignee || !priority) {
            validationErrors.value.push(`⚠️ Line ${lineNumber} is missing required fields (epic, assignee, priority).`);
            return;
        }

        if (!priorityMap.hasOwnProperty(priority)) {
            validationErrors.value.push(`⚠️ Invalid priority at line ${lineNumber}: "${priority}".`);
            return;
        }

        if (!statusMap.hasOwnProperty(status)) {
            validationErrors.value.push(`⚠️ Invalid status at line ${lineNumber}: "${status}".`);
            return;
        }

        if (!parsedListAssignee.value.hasOwnProperty(assignee)) {
            validationErrors.value.push(`⚠️ Assignee "${assignee}" at line ${lineNumber} not found.`);
            return;
        }
...
const submitFile = async () => {
    ...
    try {
        ...
        for (const task of tasks.value.filter(t => t.type === 0)) {
            const response = await axios.post(`/api/pm/${props.projectId}/tasks/store`, {
        ...
        for (const task of tasks.value.filter(t => t.type === 1)) {
            ...
            if (!epicId) {
                toastr.error(`⚠️ Epic "${task.parent_name}" not found, cannot create task "${task.name}".`);
                continue;
            }
